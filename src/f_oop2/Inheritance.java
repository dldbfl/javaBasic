...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * 하나의 클래스만 상속받을 수 있다.
		 * 상속받지않ㅇ는 모든 클래스는 Object 클래스를 상속받는다.
		 * 자식클래스는 부모클래스의 멤버외의 새로운 멤버를 갓질수있으므로 
		 * 자식클래스는 부모클래스보다 크거나 같다.
		 * 두개 이상의 클래스를 만드는데 공통된 멤버가 있는 경우 부모 클래스로 만든다.
		 * 
		 * << super >> 
		 * 부모클래스의 인스턴스 주소가 저장된 변수이다.
		 * 부모클래스의 멤버와 자식클래스의 멤버의 이름이 중복될 때 둘을 구분하기 위해 사용한다.
		 * 
		 * << super() >>
		 * 생성자에서 부모클래스의 생성자를 호출하기 위해 사용한다.
		 * 자식클래스 생성자의 첫줄에 super()가 있어야한다.
		 * 없으면 컴파일러가 자동으로 추가한다.
		 * 
		 * << 오버라이딩 >>
		 * 상속받은 메서드의 내용을 재정의 하는것이다.
		 * 상속받은 메서드와 리턴타입, 메서드명, 파라미터가 같아야한다.
		 * 상속받은 메서드보다 좁은 범위으 접근제어자로 변경할 수 없다.
